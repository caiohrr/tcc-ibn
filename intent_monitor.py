...
class IntentMonitor:
    ...
    def check_memory_usage(self, intent):
        """Checks a host's memory usage by parsing the 'free' command."""
        host_id = intent['target']
        max_mem = intent.get('value', 80)  # default threshold 80%
        
        host = self.net.get(host_id)
        
        # Run the 'free' command
        result = host.cmd("free")
        
        try:
            # Find the 'Mem:' line in the output
            mem_line = re.search(r"Mem:\s+(.*)", result)
            if not mem_line:
                print(f"[ERROR] Could not find 'Mem:' line in 'free' output for {host_id}")
                return False

            # Split the 'Mem:' line by spaces
            # Output: <total> <used> <free> <shared> <buff/cache> <available>
            parts = mem_line.group(1).split()
            
            total_mem = float(parts[0])
            available_mem = float(parts[5])
            
            # Calculate 'used' memory in a modern way (total - available)
            # This correctly accounts for buffers and cache.
            used_mem = total_mem - available_mem
            
            if total_mem == 0:
                 print(f"[ERROR] Total memory reported as 0 for {host_id}")
                 return False

            usage_percent = (used_mem / total_mem) * 100.0
            
            print(f"[INFO] Memory usage for {host_id}: {usage_percent:.2f}%")

            if usage_percent <= max_mem:
                return True
            else:
                print(f"[WARN] Memory usage ({usage_percent:.2f}%) exceeded threshold ({max_mem}%)!")
                return False
                
        except (ValueError, IndexError, TypeError) as e:
            print(f"[ERROR] Could not parse 'free' output for {host_id}: {e}\nOutput was:\n{result}")
            return False
    ...
    def recover_memory_usage(self, intent):
        ...
        try:
            # Command to get top 3 processes by memory:
            # -bn1: batch mode, 1 iteration
            # tail -n +8: Skip the 7 header lines
            # awk: Print PID (col 1), %MEM (col 10), and Command (col 12)
            # sort -k2 -nr: Sort by the 2nd column (%MEM), numerically (n), in reverse (r)
            # head -n 3: Take just the top 3
            command = "top -bn1 | tail -n +8 | awk '{print $1, $10, $12}' | sort -k2 -nr | head -n 3"
            ...
            for i, line in enumerate(top_processes):
                if not line.strip():
                    continue
                try:
                    # Parse the line, e.g., "5678 45.1 firefox"
                    parts = line.split()
                    pid = parts[0]
                    mem_percent = parts[1]
                    comm = parts[2]
