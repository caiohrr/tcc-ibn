...
class MonitoringPlugin(PluginInterface):
    ...
    @abstractmethod
    def generate_monitoring_code(self, topology: 'Topology', params: Dict[str, Any]) -> List[str]:
        """Generate code to monitor aspects of the network."""
        pass
            ...
            try:
                ...
                for name, obj in inspect.getmembers(module):
                    if (inspect.isclass(obj) and 
                        ...
                        elif isinstance(plugin_instance, MonitoringPlugin):
...
class MininetScriptGenerator:
    ...
    def generate(self, topology: Topology, output_file: str = "topology.py"):
        ...
        with open(output_file, "w+", encoding='utf-8') as mn_file:
            ...
            monitoring = Monitoring(topology, self.plugin_manager)
            monitoring_code = monitoring.generate_monitoring_code()
            if monitoring_code:
                mn_file.write("\tinfo('*** Starting monitoring\\n')\n")
                for line in monitoring_code:
                    mn_file.write(f"\t{line}\n")
                mn_file.write("\n")
...
# ============================= Monitoring ==============================
class Monitoring:
    """Basic monitoring module that generates code to verify JSON-configurable parameters.
    It includes basic checks for standard components and supports extensions via plugins.
    """
    
    def __init__(self, topology: 'Topology', plugin_manager: PluginManager):
        self.topology = topology
        self.plugin_manager = plugin_manager
    
    def generate_monitoring_code(self) -> List[str]:
        """Generate monitoring code for basic JSON parameters and plugins."""
        code = []
        param_map = {
            'bandwidth': 'bw',
            'delay': 'delay',
        ...
        code.append("info('*** Monitoring configured parameters\\n')")
        
        # Basic monitoring for hosts (IP and MAC)
        for host in self.topology.hosts:
            ...
            if 'ip' in host:
                expected_ip = host['ip']
                code.append(f"{hid}_ip = {hid}.cmd('ip addr show {iface} | grep inet | awk \\'{{print $2}}\\' | cut -d/ -f1').strip()")
                code.append(f"if {hid}_ip != '{expected_ip}':")
                code.append(f"    info(f'Warning: IP mismatch for {hid}: expected {expected_ip}, got {{{hid}_ip}}')")
            
            if 'mac' in host:
                expected_mac = host['mac']
                code.append(f"{hid}_mac = {hid}.cmd('ip link show {iface} | grep ether | awk \\'{{print $2}}\\'').strip()")
                code.append(f"if {hid}_mac != '{expected_mac}':")
                code.append(f"    info(f'Warning: MAC mismatch for {hid}: expected {expected_mac}, got {{{hid}_mac}}')")
        
        # Basic monitoring for connections/links (parameters like bandwidth, delay, etc.)
        for conn in self.topology.connections:
            endpoints = conn.get('ENDPOINTS', [])
            params = conn.get('PARAMS', {})
            
            if len(endpoints) == 2 and params:
                a, b = endpoints
                code.append(f"links = net.linksBetween({a}, {b})")
                code.append("if links:")
                code.append("    link = links[0]")
                for k, v in params.items():
                    param_name = param_map.get(k.lower(), k.lower())
                    code.append(f"    if '{param_name}' in link.intf1.params:")
                    code.append(f"        actual = link.intf1.params['{param_name}']")
                    # Handle string values (e.g., '2ms') differently to avoid extra quotes
                    if isinstance(v, str):
                        code.append(f"        if actual != '{v}':")
                    else:
                        code.append(f"        if actual != {v}:")
                    code.append(f"            info(f'Warning: {k} mismatch for link {a}-{b}: expected {repr(v)}, got {{actual}}')")
        
        # Monitoring for custom components (if the component plugin supports it)
        for component_type, components in self.topology.custom_components.items():
            plugin = self.plugin_manager.component_plugins.get(component_type)
            if plugin and hasattr(plugin, 'generate_monitoring_code'):
                for component in components:
                    custom_code = plugin.generate_monitoring_code(component)
                    if custom_code:
                        code.extend(custom_code)
        
        # Additional monitoring from dedicated monitoring plugins (configured in JSON PLUGINS)
        for config in self.topology.plugins_config:
            plugin_name = config.get("name")
            plugin_params = config.get("params", {})
            plugin = self.plugin_manager.get_plugin(plugin_name)
            if plugin and isinstance(plugin, MonitoringPlugin):
                plugin_code = plugin.generate_monitoring_code(self.topology, plugin_params)
                if plugin_code:
                    code.extend(plugin_code)
